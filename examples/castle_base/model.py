...
class P(Params):
    ...
    **Square to the front, and that is a decision rather than a default.** It was
    ...
    sign_w: float = 40.0
    """The board. It was 32, which was NOT an arbitrary number even though it
    was written as one: `sign_gap + sign_post_w` is exactly 32, so the board
    finished flush with the outside of its posts. The photographed KATHY'S sign
    is that version and has no overhang at all.

    40 is what "DRACULA'S" costs — nine characters where "KATHY'S" had seven,
    and `check()` refuses anything that runs the type into the border. Shrinking
    the type instead is not available: it takes the mean stroke under the
    one-and-a-half extrusions where routed lettering stops reading as routed.

    So the board now overhangs its posts by 4 mm a side, and that is the ONLY
    place the extra width could have gone. Keeping it flush would mean
    `sign_gap = 34` — which moves the pegs, and the sockets are cut in a base
    that is already printed. **`sign_gap` is frozen by a physical object**, so
    widening the board can only ever become overhang from here. Real signs
    overhang their posts anyway."""
    sign_h: float = 15.0
    sign_t: float = 3.6
    ...
    **Scale is a deliberate lie, and the same one the cobbles tell.** At the
    ...
    sign_lines: tuple = ("DRACULA'S", "CASTLE")
    sign_type: float = 5.4
    sign_lead: float = 2.4
    ...
    **Unless the sign is inked rather than printed in two colours**, in which
    case the whole objection evaporates: a pen colours what it touches, not what
    shares a layer with it. The frame was sacrificed to a constraint of the
    filament change, and choosing the pen instead hands it straight back.
    ...
    **`sign_letter_relief` depends on whether the sign is two-colour, and that is
    ...
    | one colour, inked | **3 layers +** | the pigment carries it; see the floor below |
    | one colour, bare | **~12 layers** (2.4 mm) | shadow is the only thing making it read |
    ...
    **Inking is the third route and it needs no pause at all.** The type is
    RAISED, so a paint pen or a marker dragged across the letter tops colours
    them and nothing else. That buys the same thing a second filament buys —
    contrast — so the twelve-layer shadow argument goes away just as completely,
    and it does it on a single-filament print with no purge bleed to recover
    from.

    Its floor is set by something different, though, and is UNMEASURED here: the
    relief has to keep the pen's tip off the board between the stems. At this
    type size the gaps and counters are only a millimetre or two, so 3 layers is
    likely marginal for a round bullet tip that can dip into them, and a flat or
    chisel tip that bridges rather than dips is the safer tool. Depth is cheap in
    this regime — nothing above the board face costs a colour change — so if in
    doubt use more of it. Nobody has tried it on this part; the number that
    belongs in this table is still missing.
...
def check(part: Part, p: P) -> None:
    ...
    if letter_layers < 8:
        print(f"  note: the lettering is {letter_layers:.0f} layers deep, which "
              f"assumes CONTRAST — either a filament change at z {p.sign_t:.1f}, "
              f"or inking the raised letter tops with a paint pen. Printed in "
              f"one bare colour it wants about 12, because then the shadow is "
              f"the only thing making it read.")
