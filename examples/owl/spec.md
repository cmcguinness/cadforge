...
A creepy owl that stares at you, cut as a flat plate and slotted into a printed branch. A tea light sits on a shelf behind its head, level with the eyes.

## What it is for

A Halloween ornament for a shelf or a mantel, looked at from the front in a dim room. Two pieces, printed separately and assembled dry: the owl plate, and a branch it perches on. It takes the same battery tea light as every other lit part here.
...
- [ ] **The shelf is not visible from the front.** The body is far wider than the puck at that height, so this is a matter of not making the shelf wider than it needs to be.
...
- [ ] **Two pieces, each one connected solid and a watertight mesh**, declared through `pieces()`.
...
- [ ] Fits the configured bed; `PRINT_ROTATION` stated for both pieces, and the plate is exported lying flat.
...
- [ ] **Whether the beak is subordinate or distracting.** Geometry says the opaque base shadows it completely, but that is line-of-sight — bounce and the diffuser will lift it off black by some unknown amount.
