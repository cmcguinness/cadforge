...
- **Image metadata is stripped too, and it was the bigger leak.** A photo's EXIF carries the second it was taken and the camera's serial number; images were copied through untouched for a long time. `cadkit/imgmeta.py` rewrites every published JPEG, PNG and GIF without it, losslessly (the compressed data is copied byte for byte), keeping only orientation and colour profiles. An image format it cannot strip is refused before anything is copied — convert it, do not work around the refusal. Two further date carriers are handled the same way: a render sheet's timestamp banner is cropped off the published copy (`render.strip_banner`), and a `.3mf` loses its creation/modification dates, archive timestamps, MakerWorld account ID and print-host credentials (`cadkit/threemf.py`).
